...
function base_actions {
    mkdir -p ~/OpenDelta/code/kernel/img/
    mkdir -p ~/OpenDelta/code/kernel/obj/
}
...
function build-kern-64 {
    ...
    ld.lld -m elf_x86_64 -z noexecstack -T link.ld -Map kernel.map "${OBJS[@]}"
