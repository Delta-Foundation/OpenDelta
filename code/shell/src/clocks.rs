...
fn print_clock(time: String, date: String) {
    // Получаем все строки для каждой цифры
    let time_lines: Vec<Vec<String>> = time.chars()
        .map(|c| big_char(c).lines().map(|s| s.to_string()).collect())
        .collect();

    // Выводим каждую строку по отдельности
    for i in 0..5 {
        for line in &time_lines {
            if let Some(char_line) = line.get(i) {
                print!("{}", char_line);
            }
            print!(" "); // Пробел между цифрами
        }
        println!();
    }
